...
fn main() {
    ...
    // 給油量の必要最小限は L-K
    // このうちiリットル目をどのガソリンスタンドで給油するか考える
    // そのために、iリットル目のガソリンを給油できる場所を考える
    // まず、ガス欠になったら走れないので、iリットル目を給油しなかったら K+i-1キロしか走れない
    // さらに、タンク容量 K を溢れさせないために、iリットル目を補給するには iリットル走らないといけない
    // つまり i リットル目のガソリン補給地点までの距離を d_i とすると
    // i <= di <= k+i-1 が必要であり、逆にこれを満たしている di 上にあるガソスタでなら補給する方法は存在する
    // iリットル目のガソリンを安くするには、di として取れるガソスタのうち最小値のところを選べば良い
    // で、この選択を 1 <= i <= L-K の全ての i で個別にできるか（できるならそれが最安だが、実現可能であることは自明ではない）
    // 結論は、できる
    // iリットル目の補給地点を e_i としたとき、 e_i <= e_{i+1} を満たしていなければならないが、この制約下でも、
    // 必ず最安値を選ぶことができる
    // もしできないと仮定するなら、初めて理論上の最安値を選べない e_{i+1} が存在する。つまり i <= d_{i+1} <= e_i-1 にあるガソスタの中に
    // e_{i+1} のガソスタより安い場所がある。でもこれは e_i のガソスタよりも安く、i リットル目の補給先としても選択可能な地点である
    // これは i が最安値を選んでないことを意味するので、 i+1 が初めて最安値を選べなかったという仮定に矛盾する

    let stands = get_vec(&ac, l);
    let segtree = SegmentTree::init(&stands);
    let ans: i64 = if l <= k {
        0
    } else {
        let val: Option<i64> = (1..=(l - k)).try_fold(0, |acc, i| {
            let min_v = segtree.fold(i, k + i);
            if min_v == usize::MAX {
                None
            } else {
                Some(acc + min_v as i64)
            }
        });
        val.unwrap_or(-1)
    };
...
fn get_vec(ac: &[(usize, usize)], l: usize) -> Vec<usize> {
    let mut res = vec![usize::MAX; l + 1];
    for &(a, c) in ac.iter() {
        res[a] = res[a].min(c);
    }
    res
