...
fn main() {
    ...
    // dp[1], ..., dp[n] を持つセグ木
    let mut segtree: SegmentTree<usize> = SegmentTree::init(&vec![Monoid::id(); n]);
    let mut dp: Vec<usize> = vec![Monoid::id(); n + 1];
    dp[1] = 0;
    segtree.update(0, 0);

    for i in 2..=n {
        // x_posl >= x_i - r を満たす最小の posl
        // x_posr <= x_i - l を満たす最大の posr
        let posl = {
            let posl = x.partition_point(|&xl| xl < x[i - 1] - r);
            // x_i - r 以上のところに自分より前の足場が一個もない時
            if posl == i {
                None
            } else {
                Some(posl)
        ...
        let posr = {
            let posr = x.partition_point(|&xr| xr <= x[i - 1] - l);
            // xi - l < xr となる最小の　r　が 0、つまり、　xi - l >= xr となる r が存在しない時
            if posr == 0 {
                None
            } else {
                Some(posr - 1)
        ...
        //println!("i: {}, left: {:#?}, right: {:#?}", i, posl, posr);
        match (posl, posr) {
            (Some(left), Some(right)) if left <= right => {
                dp[i] = segtree.fold(left, right + 1) + 1;
                //println!("dp[i]: {}", dp[i]);
